...
function export_report($report)
{
	...
	$count_va_site1_deleted	=	0;
	$count_va_site2_deleted	=	0;
	...
	foreach ($csv as $key => $csvuser)
		{
            ...
            if (empty($vA))
            {	// VA for Site1 does'nt exist, no need to do anything


                    $accounts_site1 = array	(
        									"beneficiary_name"  => $account_nammes_arr[0] ,
        									"va_id"             => "Does not exist" ,
        									"account_number"    => "Does not exist" ,
        									"va_ifsc_code"      => "Does not exist" ,
        									);
            ...
            {   // the account for Site1 already exists, details got by function getvAccountGivenId above
                ...
                $accounts_site1 = array	(
                                        "beneficiary_name"  => $account_nammes_arr[0] ,
                                        "va_id"             => "To Be Deleted" ,
                                        "account_number"    => "To Be Deleted" ,
                                        "va_ifsc_code"      => "To Be Deleted" ,
                                        );
                $accounts[$site_names_arr[0]]    = $accounts_site1;
                // count the item that needs to be deleted
                $count_va_site1_deleted += 1;
            }
