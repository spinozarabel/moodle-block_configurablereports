...
function export_report($report)
{
    ...
    $users_from_report   =  $matrix;  # instead of downloading and parsing, we are reusing



    array_walk($users_from_report, function(&$a) use ($users_from_report)
		{
			$a = array_combine($users_from_report[0], $a);
		});
	array_shift($users_from_report); # remove column header
	// find number of entries extracted from users_from_report into array
    $usercount = count($users_from_report);
	//----------------------------------- end of section 2 --------------------------------------->
	...
	foreach ($users_from_report as $key => $user):
        ...
        else
        {
            ...
            // read in the JSON encoded data or set it to blank if empty or has meaningless phrases
            $fees_json		= $user_profile_fees->data ?? "";

            if (empty($fees_json) || stripos($fees_json, "paid" !== false)) $fees_json = "[]";
